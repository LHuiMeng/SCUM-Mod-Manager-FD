...
void CreateAndAttachConsole() {
  if (::AllocConsole()) {
    // 控制台按 UTF-8 解释（Dart/Flutter 日志为 UTF-8，默认 OEM/GBK
    // 代码页会致中文乱码 —— 显式切到 CP_UTF8）。
    ...
    if (freopen_s(&unused, "CONOUT$", "w", stderr)) {
      // 笔误修复：原版 _dup2(_fileno(stdout), 2) —— 应为 _fileno(stderr)，
      // 否则 fd 2（stderr 描述符）被拷贝成 stdout 的值，stderr 实际未重定向。
